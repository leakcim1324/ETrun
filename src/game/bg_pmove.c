...
static void PM_AccelerateW( vec3_t wishdir, float wishspeed, float accel )
{
	...
	float dot;
	// Not partially 'borrowed' from warsow at all
	float turnaccel = 5.0f;			// Sharpness
	float backtosideratio = 0.5f;	// Dir
	...
	{
		float accelspeed = curspeed + 320 * pml.frametime;
		if( accelspeed < wishspeed )
			wishspeed = accelspeed;
	}
	else
	{
		wishspeed = max( curspeed, 320 );
	}
	...
	accelspeed = turnaccel * 320 /* wishspeed */ * pml.frametime;
	if( accelspeed > addspeed )
		accelspeed = addspeed;
