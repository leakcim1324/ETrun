...
static void CG_DrawOB(void)
{
	...
	if (!tj_drawOB.integer || cg_thirdPerson.integer)
		return;
...
static void CG_DrawAccelBar( rectDef_t *rect ) {
	...
	thistime = trap_Milliseconds();
	speednow = VectorLength(cg.predictedPlayerState.velocity);

	if(thistime > (lasttime + 250)) 
	{

		speedpast = VectorLength(cg.predictedPlayerState.velocity);
...
{		
	...
	float angle = cg.predictedPlayerState.viewangles[YAW];
...
static void CG_DrawYawDirection(void)
{
	...
	{
		pastangle = cg.predictedPlayerState.viewangles[YAW];
		lasttime = thistime;
	}
...
///////////////////////////////////////////////////////////////////////////
	...
	Com_sprintf(status, sizeof(status), va("Jump count: %i", cg.predictedPlayerState.curWeapHeat));
...
static void CG_DrawJumps (void)
{
	...
	speed = sqrt(cg.predictedPlayerState.velocity[0]*cg.predictedPlayerState.velocity[0] + cg.predictedPlayerState.velocity[1]*cg.predictedPlayerState.velocity[1]);
	thisJumpCount = cg.predictedPlayerState.curWeapHeat;
